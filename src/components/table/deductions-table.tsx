...
import { deductionLines, deductionTypes, type DeductionType } from "@/mocks/deductions";

const typeStyles: Record<DeductionType, string> = {
  "Income Tax": "bg-red-500/15 text-red-500 border-red-500",
  "National Insurance": "bg-yellow-500/15 text-yellow-600 border-yellow-500",
  Pension: "bg-cyan-500/15 text-cyan-600 border-cyan-500",
  "Student Loan": "bg-violet-600/15 text-violet-600 border-violet-600",
  "Season Ticket Loan": "bg-gray-500/15 text-gray-500 dark:text-white border-gray-400",
};
...
const DeductionsTable = () => {
  ...
  const [type, setType] = useState<"all" | DeductionType>("all");

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return deductionLines.filter((line) => {
      const matchesQuery = !q || line.employeeName.toLowerCase().includes(q);
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((line) => (
                <TableRow key={line.id}>
                  ...
                  <TableCell className="py-3 px-4 border-b border-neutral-200 dark:border-slate-600 text-end font-medium whitespace-nowrap text-red-500">
                    −{gbp(line.amount)}
                  </TableCell>
                </TableRow>
              ))
