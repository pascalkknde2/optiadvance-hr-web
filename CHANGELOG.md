...
- Initialized the Next.js 16.3.5 / React 19.3.0 App Router project (TypeScript strict mode, Tailwind CSS v4) with the `src/{app,features,components,design-system,lib,mocks,tests}` boundaries from the former structure blueprint materialized as real directories.
...
- Initialized Git; pushed `main` (default), `develop` and `staging` branches to GitHub, with force-push and deletion blocked on `main`.

### Changed

- Switched the package manager from npm to pnpm 10.20.0 (pinned via `packageManager` in `package.json`); replaced `package-lock.json` with `pnpm-lock.yaml`.
